// 界断粮(界徐晃·主动技/转化,OL 现行界版):
//   你可以将一张黑色基本牌或黑色装备牌当【兵粮寸断】使用。
//   若你本回合未造成过伤害,你使用【兵粮寸断】无距离限制。
//
// 与标版差异:
//   - 转化范围:同为"黑色基本牌或装备牌"(与标版一致)
//   - 距离放松:标版为"目标手牌数≥自己则无距离限制";
//     界版改为"本回合未造成过伤害则无距离限制"(OL 现行界版规则)
//
// 实现策略(独立 use,同标版):
//   兵粮寸断.use 的距离校验硬编码 effectiveDistance<=1,无法承载"无距离限制"的放松规则。
//   且断粮转化的卡不是"兵粮寸断"卡(而是黑色基本/装备)。故采用独立 use action:
//   validate 校验黑色基本/装备 + 放松距离;execute 直接放置 name='兵粮寸断' 的延时锦囊。
//   判定/跳过摸牌阶段的效果由现有 兵粮寸断.ts 的 hooks 处理(它们只认 pendingTricks.name)。
//   无次数限制(描述明确)。
//
// 本回合伤害追踪(界版核心差异):
//   - 造成伤害 after-hook:source=ownerId 且 amount>0 → turn.vars[DMG_VAR]=true
//   - 同步经「回合用量」atom 投影到 view.players[ownerId].turnUsage,供前端 targetFilter 读取
//   - 由「回合结束」atom 自动清空 turn.vars / view.turnUsage,天然每回合重置
//
// 距离规则:
...
const DMG_VAR = '界断粮/damageDealt';

/** 界断粮可用牌:黑色基本牌或黑色装备牌(同标版范围) */
function isDuanliangCard(card: Card | undefined): boolean {
  if (!card) return false;
  if (card.color !== '黑') return false;
  return card.type === '基本牌' || card.type === '装备牌';
}
...
export function createSkill(id: string, ownerId: number): Skill {
  return {
    ...
    name: '界断粮',
    description:
      '将一张黑色基本牌或黑色装备牌当【兵粮寸断】使用;若本回合未造成过伤害,使用时无距离限制',
  };
}
...
export function onInit(skill: Skill, state: GameState): () => void {
  ...
  registerAction(
    ...
    (state: GameState, params: Record<string, Json>): string | null => {
      ...
      if (!isDuanliangCard(state.cardMap[params.cardId]))
        return '需要黑色基本牌或装备牌';
      ...
      // 距离规则:本回合未造成过伤害 → 无距离限制;否则须 ≤1
      const noDamage = !state.turn.vars[DMG_VAR];
      const inRange = noDamage || effectiveDistance(state, ownerId, target) <= 1;
...
export function onMount(skill: Skill, api: FrontendAPI): (() => void) | void {
  api.defineAction('use', {
    ...
    prompt: {
      ...
      title: '界断粮:将黑色基本牌或装备牌当兵粮寸断使用',
      cardFilter: { filter: (c: Card) => isDuanliangCard(c), min: 1, max: 1 },
      targetFilter: {
        min: 1,
        max: 1,
        // 距离放松:本回合未造成过伤害 → 无限制;否则 ≤1
        // turnUsage[DMG_VAR] 由「回合用量」atom 实时同步,与后端 turn.vars 一致。
        filter: (view: GameView, t: number) => {
          ...
          const noDamage = !view.players[me]?.turnUsage?.[DMG_VAR];
          if (noDamage) return true; // 无距离限制
