...
// ============================================================
// 技能描述(三国杀官方规则,见 docs/research/武将技能/吴国/孙权.md):
//   制衡(孙权):
//     - 触发时机:出牌阶段
//     - 发动条件:有牌可以弃置
//     - 效果:弃置任意数量的牌(手牌或装备牌),然后摸等量的牌
//     - 限制:每回合限一次
//     - 备注:可以弃置 0 张(但无意义);弃置的牌进入弃牌堆;摸的牌来自牌堆顶
//     - FAQ:可以弃置装备区(包括刚装备的武器);**不能**弃置判定区的牌
//
// 关键原子操作:
//   use 路径:
//     pushFrame → 弃置(cardIds) → 摸牌(count = cardIds.length) → popFrame
//
// 关键时机:
//   - 出牌阶段限一次
//
...
//   2. **弃置数量错误**:规则是"任意张牌(0~所有),然后摸等量的牌",
//      当前固定单张 + 固定摸 2 张,完全偏离规则!
//      应:弃置 N 张(N≥1,可选手牌/装备),然后摸 N 张。
//   3. **弃牌范围错误**:规则是"手牌或装备牌",**不能**弃置判定区(FAQ 明确)。
//      当前只支持单张手牌。onMount 的 prompt 写死 min:1, max:1。
//   4. validate 未检查 cardId 是否在手牌/装备区中(防御缺失)。
//   5. validate 仅检查 cardId 为 string,未检查长度数组形式(规则允许多张)。
//   6. **摸牌数量硬编码 count=2**:应与弃置数量 N 相等,不是固定 2。
// ============================================================
// ============================================================
