...
export function onInit(skill: Skill, state: GameState): (() => void) | void {
  ...
  registerAction(
    ...
    async (st: GameState, params: Record<string, Json>) => {
      ...
      // 必须在 pushFrame 之前:markOncePerTurn 在入口同步写 vars,若 pushFrame 先 await,
      // 其 applyAtom 会让出事件循环,dispatch 是 fire-and-forget,前端可在 vars 写好前
      // 再次发 use → validate 的 usedThisTurn 仍为 false → 可重复发动。
