...
//   · 杀:须尊重出杀次数限制(canSlash 预检;计数由 chargeOnSettle 的 onSettle 负责)。
 ...
 * 保证激昂/集智/界求援等技能事件一致。计入出杀次数：virtual+charge 下 runUseFlow 的
 * onSettle 回调(由 chargeOnSettle 传入)触发 杀.onSettle→incSlashUsed（与下方第 284 行内联注释一致）。
 */
