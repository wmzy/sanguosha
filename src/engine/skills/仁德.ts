...
export function onInit(skill: Skill, state: GameState): () => void {
  ...
  registerAction(
    ...
    (state: GameState, params: Record<string, Json>) => {
      ...
      const targetsLegal =
        hasCards &&
        normalized.every((t) => t.target !== ownerId && state.players[t.target]?.alive === true);
      const ok =
        myTurn &&
        inActPhase &&
        free &&
    ...
    async (state: GameState, params: Record<string, Json>) => {
      ...
      const allocParam = params.allocation as
        | Array<{ target: number; cardIds: string[] }>
        | undefined;
      const rawTargets = params.targets as
        | Array<{ target: number; cardIds: string[] }>
        | number[]
        | undefined;
      ...
      if (
        Array.isArray(allocParam) &&
        allocParam.length > 0 &&
        Array.isArray((allocParam[0] as { cardIds?: unknown }).cardIds)
      ) {
        targets = allocParam;
      } else if (
        Array.isArray(rawTargets) &&
        rawTargets.length > 0 &&
        Array.isArray((rawTargets[0] as { cardIds?: unknown }).cardIds)
      ) {
