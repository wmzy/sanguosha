...
export async function markOncePerTurn(state: GameState, ownerId: number, skillName: string): Promise<void> {
  state.players[ownerId].vars[usedThisTurnKey(skillName)] = true;
  await applyAtom(state, { type: '回合用量', player: ownerId, key: usedThisTurnKey(skillName), value: true });
}
...
export function activeUnlessUsedThisTurn(skillName: string): (ctx: ActionContext) => boolean {
  return (ctx: ActionContext): boolean =>
    defaultPlayActive(ctx) && !ctx.view.players[ctx.perspectiveIdx]?.turnUsage?.[usedThisTurnKey(skillName)];
}
