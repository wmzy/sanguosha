...
//   - apply: 弹出牌堆顶(末尾),判定点数是否与已有创牌重复:
//            · 不重复 → 追加到创牌列表;
//            · 重复   → 移去此牌(进弃牌堆,不留武将牌)——官方:"否则移去此牌"。
//            重复判定结果写入 localVars 供不屈 hook 读取决定存活/死亡。
...
//     创牌列表不投影到 GameView 字段(buildView 不投影 player.vars['不屈/创牌'],
...
export const 置创牌: AtomDefinition<{ player: number }> = {
  ...
  apply(state, atom) {
    ...
    if (duplicate) {
      // 点数重复:移去此牌(进弃牌堆),不置于武将牌上——官方"否则移去此牌"
