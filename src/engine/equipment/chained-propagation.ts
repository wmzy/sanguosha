...
// P5-T1：chained 走 Mark 体系。读 hasChained(state, player)，不再读 PlayerState.chained 字段。
//
// 注意（占位实现的已知限制）：
...
// 也应在同链其他成员之间互传），而本占位实现只在"目标 chained"时才追加，
...
import { hasChained, getChainedOthers } from '../mark';
...
export function register(registry: HookRegistry): void {
  registry.register({
    ...
    onAfter({ state, atom }: { state: GameState; atom: Atom }) {
      ...
      if (!hasChained(state, target)) return {};
      ...
      const additionalAtoms: Atom[] = getChainedOthers(state, target)
        .map((name) => ({
          type: '造成伤害' as const,
          target: name,
          amount,
