// 座位区样式:弧形排列 + 中央区域行(处理区/弃牌堆) + 动画状态。
// 武将座位卡样式已在 PlayerSeatView.tsx 本地化;本文件仅保留仍有共享消费方的条目:
//   - 座位环布局(SeatArcLayout)+ 牌堆/弃牌堆行(ZoneInfoBar)
//   - 大卡(自己)可点选/被选为目标态(GameView 底栏 PlayerCardLarge 复用)
//   - 处理区标签(ZoneInfoBar)+ 动画状态类(GameView/PlayerCardLarge/GameHeader)
...
// ─── 大卡(自己)目标选择态:GameView 底栏 PlayerCardLarge 外层复用 ───
// (可选自己为目标:铁索连环含自己;座位卡同款交互样式在 PlayerSeatView.tsx 本地)
