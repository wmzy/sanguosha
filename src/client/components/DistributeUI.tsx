...
export function DistributeUI({
  ...
  for (const a of allocations) perTargetCount.set(a.target, (perTargetCount.get(a.target) ?? 0) + a.cardIds.length);
    ...
    return (
      <div className={promptActions} style={{ flexDirection: 'column', alignItems: 'stretch', gap: 8 }}>
        <div className={promptHint}>
          {prompt.title}{rangeHint} · 已选 {totalSelected}
        </div>
        <div className={actionRow}>
          <button className={promptBtn} onClick={onClear} disabled={totalSelected === 0}>清空</button>
          <button className={promptBtnPrimary} onClick={onSubmit} disabled={!canSubmit}>确认({totalSelected})</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={promptActions} style={{ flexDirection: 'column', alignItems: 'stretch', gap: 8 }}>
      <div className={promptHint}>
        ...
        {selected.size > 0 && <span className={allocSelHint}> · 已选 {selected.size} 张,点击玩家分配</span>}
      </div>
      ...
      <div className={actionRow}>
        <button className={promptBtn} onClick={onClear} disabled={totalAllocated === 0 && selected.size === 0}>清空</button>
        {!allAllocated && (
          <button className={promptBtnPrimary} onClick={onSubmit} disabled={totalAllocated < minTotal}>
            提交分配({totalAllocated})
          </button>
        )}
      </div>
...
const allocSelHint = css`
  color: #2ecc71; font-weight: normal; font-size: 12px;
`;
