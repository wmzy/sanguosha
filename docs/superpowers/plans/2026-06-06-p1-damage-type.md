...
**Architecture:** 单点修改 `engine/atoms/damage.ts`：扩展 `damage` atom 类型 + 写入 server event payload + 实现 `chainPropagation` 副作用（连环状态被 §0.3 标记为缺失，本 Task **不引入** chained 字段，只为后续 P1-B 留接口；`chainPropagation` 在 type=thunder/normal 时按 `chained: boolean` 字段传播，没有该字段则不传播——纯依赖驱动）。

**Tech Stack:** TypeScript 5.9 + vitest 4.1 + pnpm。Atom 注册走 `engine/atom.ts:registerAtom`；事件订阅走 `engine/skill-hook.ts:registerAtomHook`。
...
**Files:**
- Modify: `engine/types.ts:199`（Atom 联合加 type 字段）
- Modify: `engine/atoms/damage.ts:7-26`（apply/toEvents 读写 type）
...
```bash
git add engine/types.ts engine/atoms/damage.ts tests/atoms/damage-type.test.ts
...
**Files:**
- Create: `engine/skills/tengjia.ts`（v3 registerAtomHook 实现）
- Create: `engine/skills/daqi.ts`（v3 registerAtomHook 实现）
- Modify: `engine/skills/equipment.ts`（删旧 stub；不删装备注册，删 handler 空壳）
...
### 2.3 写 `engine/skills/tengjia.ts`
...
### 2.4 写 `engine/skills/daqi.ts`（大雾：thunder 伤害无效）
...
```ts
import { register as registerTengjia } from '@engine/skills/tengjia';
import { register as registerDaqi } from '@engine/skills/daqi';
...
```bash
git add engine/skills/tengjia.ts engine/skills/daqi.ts tests/scenarios/装备/藤甲.test.ts tests/scenarios/装备/大雾.test.ts tests/fixtures/藤甲.ts
