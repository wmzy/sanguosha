...
**Architecture:**
- **#1 RNG 统一**：改 `engine/atoms/reshuffle.ts` 用 `rng.getState()` 真实消费 nextInt 调用（与 shuffleDeck 一致）。删除合成 `state.rngState + length-1` 逻辑。
- **#2 faceDown 跳整回合**：在 `engine/phase-advance.ts:advanceToInteractivePhase` turnStart 之前检查 `state.marks[currentPlayer]` 中的 `faceDown` Mark，**直接调 nextPlayer 跳过整个回合**。Mark 在 phaseBegin 时清除（与现有 `untilTurnEnd` 语义一致）。
...
**Files:**
- Modify: `engine/atoms/reshuffle.ts:18-22`
...
### 1.3 改 `engine/atoms/reshuffle.ts:18-22`
...
### 1.6 提交
```bash
git add engine/atoms/reshuffle.ts tests/atoms/reshuffle-rng.test.ts
...
**Files:**
- Modify: `engine/phase-advance.ts:advanceToInteractivePhase` turnStart 之前加 faceDown 检查
- Modify: `engine/phase-advance.ts:processPhaseStep` phaseBegin 之前清 faceDown Mark
...
### 2.3 改 `engine/phase-advance.ts`

**先 read** `engine/phase-advance.ts:187-247` 找精确位置。
...
### 2.6 提交
```bash
git add engine/phase-advance.ts tests/integration/facedown-skip-turn.test.ts
...
**Files:**
- Create: `engine/skills/leiji.ts`（雷击 v3 hook）
- Create: `engine/skills/雷电伤害.ts`（雷电伤害 chain-propagation 钩子，已有 1C-T5 chained-propagation 可扩展）
- Modify: `engine/skills/chained-propagation.ts:filter`（确认 thunder 已在 fire/thunder 列表）
...
### 3.2 写 v3 hook：`engine/skills/leiji.ts`
...
```ts
import { register as registerLeiji } from '@engine/skills/leiji';
...
### 3.8 提交
```bash
git add engine/skills/leiji.ts tests/scenarios/群/雷击.test.ts tests/scenarios/装备/雷电-连环.test.ts tests/fixtures/雷击.ts
...
**Files:**
- Modify: `engine/skills/bagua.ts`（去掉 onBefore 占位 cancel，改成 in useCard hook 阶段）
- Create: `engine/skills/_useCardBagua.ts`（监听 becomeTarget 阶段，注入判定 prompt）
- Modify: `engine/atoms/useCard.ts` 现有 useCard 三原子：specifyTarget / becomeTarget / resolveCard（看 P0 commit 30f2d55 + 6901023 实现）
- Test: `tests/scenarios/装备/八卦阵-完整判定.test.ts`（新）

### 4.1 读 useCard 三原子现状

**先 read**:
- `engine/atoms/useCard.ts` (or wherever P0 useCard 三原子 lives)
- `engine/skills/bagua.ts` 当前实现（commit 633afb2 写的占位）
...
### 4.3 改 `engine/skills/bagua.ts`（v3 真实规则）
...
### 4.6 提交
```bash
git add engine/skills/bagua.ts tests/scenarios/装备/八卦阵-完整判定.test.ts
