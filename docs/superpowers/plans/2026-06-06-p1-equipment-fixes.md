...
**Spec:** `docs/architecture/引擎架构.md` §4.1（validate 硬编码）+ §4.2（八卦阵）+ §4.3（4 武器 stub）。
...
**Files:**
- Modify: `src/src/engine/types.ts:390-450`（SkillDef 加 convertible 字段）
- Modify: `src/src/engine/validate.ts:73-121`（getSkillConvertedCards 改读 convertible 字段）
- Modify: `src/src/engine/skills/shu.ts:83-200`（武圣/龙胆/奇才 加 convertible 字段）
- Modify: `src/src/engine/skills/wei.ts:280-360`（倾国 加 convertible 字段）
...
### 1.3 改 `src/src/engine/types.ts:390-450` SkillDef
...
### 1.4 改 `src/src/engine/validate.ts:73-121`
...
> **expr 评估**：复用 `src/src/engine/expr.ts:evaluate`（已存在）。如 evaluate 签名不同，按现存实际签名调整。

### 1.5 给武圣/龙胆/奇才/倾国加 convertible 字段

`src/src/engine/skills/shu.ts:83` 武圣：
...
`src/src/engine/skills/shu.ts:127` 龙胆：
...
```bash
git add src/src/engine/types.ts src/engine/validate.ts src/engine/skills/shu.ts src/engine/skills/wei.ts tests/unit/validate-convertible.test.ts
...
**Files:**
- Create: `src/src/engine/skills/bagua.ts`（v3 registerAtomHook）
- Modify: `src/src/engine/skills/equipment.ts:138-162`（删旧 stub handler）
...
### 2.3 写 `src/src/engine/skills/bagua.ts`
...
### 2.4 删 `src/src/engine/skills/equipment.ts:138-162` judgeDodge 旧 stub
...
```bash
git add src/src/engine/skills/bagua.ts src/engine/skills/equipment.ts tests/scenarios/装备/八卦阵.test.ts
...
**Files:**
- Create: `src/src/engine/skills/qinggang.ts`
- Modify: `src/src/engine/skills/equipment.ts:17-28`（删空 stub）
...
### 3.3 写 `src/src/engine/skills/qinggang.ts`
...
```bash
git add src/src/engine/skills/qinggang.ts src/engine/skills/equipment.ts tests/scenarios/装备/青釭剑.test.ts tests/fixtures/青釭剑.ts
...
**Files:**
- Create: `src/src/engine/skills/zhangba.ts`、`src/src/engine/skills/fangtian.ts`、`src/src/engine/skills/renwang.ts`
- Modify: `src/src/engine/skills/equipment.ts:185-203`（删 3 个空 stub）
- Test: `tests/scenarios/装备/丈八蛇矛.test.ts`、方天画戟、仁王盾

### 4.1 仁王盾（最简单——onBefore cancel 黑杀）

`src/src/engine/skills/renwang.ts`:
...
`src/src/engine/skills/zhangba.ts`:
...
`src/src/engine/skills/fangtian.ts`:
...
`src/src/engine/skills/equipment.ts:185-203` 改为 `trigger: { event: 'v3HookOnly', source: 'equipment' }`。
...
```bash
git add src/src/engine/skills/renwang.ts src/engine/skills/zhangba.ts src/engine/skills/fangtian.ts src/engine/skills/equipment.ts tests/scenarios/装备/仁王盾.test.ts tests/scenarios/装备/丈八蛇矛.test.ts tests/scenarios/装备/方天画戟.test.ts
