...
**Files:**
- Modify: `engine/types.ts:198-235`（Atom 联合加 loseHealth）
- Create: `engine/atoms/loseHealth.ts`
- Modify: `engine/atoms/index.ts`（注册）
...
`engine/types.ts:198-235` 在 `heal` 后新增：

```ts
  | { type: 'loseHealth'; target: Expr<string>; amount: Expr<number> }
```

### 1.4 写 `engine/atoms/loseHealth.ts`
...
`engine/atoms/index.ts` 加 `import { register as registerLoseHealth } from './loseHealth';` 和 `registerLoseHealth();`。
...
```bash
git add engine/types.ts engine/atoms/loseHealth.ts engine/atoms/index.ts tests/atoms/lose-health.test.ts
...
**Files:**
- Modify: `engine/types.ts`（加 loseCard 变体）
- Create: `engine/atoms/loseCard.ts`
...
`engine/types.ts` 在 `moveCard` 后新增：
...
### 2.4 写 `engine/atoms/loseCard.ts`
...
`engine/atoms/index.ts` 加 `import { register as registerLoseCard } from './loseCard';` 和 `registerLoseCard();`。
...
```bash
git add engine/types.ts engine/atoms/loseCard.ts engine/atoms/index.ts tests/atoms/lose-card.test.ts
...
**Files:**
- Modify: `engine/types.ts`（加 removeSkill 变体）
- Create: `engine/atoms/removeSkill.ts`
- Modify: `engine/atoms/index.ts`
...
`engine/types.ts` 在 `addSkill` 后新增：

```ts
  | { type: 'removeSkill'; player: Expr<string>; skillId: string }
```

### 3.4 写 `engine/atoms/removeSkill.ts`
...
`engine/atoms/index.ts` 加 `import { register as registerRemoveSkill } from './removeSkill';` 和 `registerRemoveSkill();`。
...
```bash
git add engine/types.ts engine/atoms/removeSkill.ts engine/atoms/index.ts tests/atoms/remove-skill.test.ts
