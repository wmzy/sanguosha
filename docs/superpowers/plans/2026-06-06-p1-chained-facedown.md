...
**Spec:** `docs/architecture/引擎架构.md` §1.2（loseHealth / chained / Mark faceDown 决策）+ §5 T-07（Mark faceDown + untilTurnEnd）+ T-08（创牌独立牌区——本 Plan **不**做创牌，只做 Mark 体系骨架）+ §4.8（PlayerState 缺字段）。
...
- Modify: `src/src/engine/view/reducer.ts:578`（创建 player 时默认 chained=false——按 docs/architecture/引擎架构.md §0.3 已知这不是新 bug，是补默认）
...
`src/src/engine/view/reducer.ts:578`（按 docs/architecture/引擎架构.md §0.3 提示是已知位点）创建 player 时加 `chained: false` 显式字段。
