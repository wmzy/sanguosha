...
**Tech Stack:** TypeScript 5.9 + vitest 4.1 + pnpm。Mark 走新 `engine/mark.ts` 模块；chained 走字段 + `damage` 钩子扩展。
...
**Non-Goals:**
- 不做铁索连环卡牌完整流程（卡牌定义在 shared/cards/tricks.ts 已存在，但 handler 未实现——本 Plan 只做"chained 状态字段 + 连环传导钩子"，卡牌使用流程留 P2）
...
**Files:**
- Modify: `engine/types.ts:54-65`（PlayerState 加 chained 字段）
- Modify: `engine/view/reducer.ts:578`（创建 player 时默认 chained=false——按 docs/ENGINE.md §0.3 已知这不是新 bug，是补默认）
...
`engine/view/reducer.ts:578`（按 docs/ENGINE.md §0.3 提示是已知位点）创建 player 时加 `chained: false` 显式字段。
...
```bash
git add engine/types.ts engine/view/reducer.ts tests/engine-helpers.ts tests/atoms/player-chained.test.ts
...
**Files:**
- Modify: `engine/types.ts`（Atom 联合加 setChained）
- Create: `engine/atoms/setChained.ts`
- Modify: `engine/atoms/index.ts`
...
### 2.4 写 `engine/atoms/setChained.ts`
...
`engine/atoms/index.ts` 加 `import { register as registerSetChained } from './setChained';` 和 `registerSetChained();`。
...
```bash
git add engine/types.ts engine/atoms/setChained.ts engine/atoms/index.ts tests/atoms/set-chained.test.ts
...
**Files:**
- Create: `engine/mark.ts`（Mark 类型 + state.marks 字段操作函数）
- Modify: `engine/types.ts:48-88`（GameState 加 marks 字段 + Mark 类型导出）
- Modify: `engine/view/reducer.ts`（state.marks 默认空 Record）
...
`engine/types.ts:48-65` 加 marks 字段：
...
`engine/types.ts:198-235` Atom 联合新增：
...
`engine/atoms/index.ts` 加 `import { register as registerMark } from './mark';` 和 `registerMark();`。
...
```bash
git add engine/mark.ts engine/types.ts engine/atoms/mark.ts engine/atoms/index.ts engine/view/reducer.ts tests/engine-helpers.ts tests/unit/mark.test.ts
...
**Files:**
- Modify: `engine/phase-advance.ts`（在 turnStart / 阶段推进时检查 faceDown）
...
**Files:**
- Create: `engine/skills/chained-propagation.ts`（v3 registerAtomHook）
...
### 5.3 写 `engine/skills/chained-propagation.ts`
...
```bash
git add engine/skills/chained-propagation.ts tests/scenarios/装备/铁索连环.test.ts tests/fixtures/铁索连环.ts
