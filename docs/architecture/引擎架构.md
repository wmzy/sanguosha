...
```ts
// 技能模块,一个技能一个模块文件,由技能声明注册表(skills/registry.ts)聚合声明、
// skills/loaders.ts 按声明懒加载(导出 SkillModule 对象, default export)
...
for each player:
  for each skillId in player.技能列表:
    1. 通过 skills/loaders.ts 按声明动态 import() 技能模块 (skills/${skillId}.ts)
    2. skill = module.createSkill(skillId, player.name)   // 创建实例
    3. module.onInit(skill, player.name)                  // 注册 action/钩子
```
- 技能模块按**声明注册表**懒加载:`skills/registry.ts` 聚合武将(data/characters 的 path 字段)/装备(卡名)/系统/衍生四类声明,`skills/loaders.ts` 据此动态 import(vitest/浏览器走 vite-glob,Node 走绝对路径 import)
...
for each player:
  for each skillId in player.skills:
    1. 通过 skills/loaders.ts 按声明注册表动态 import() 技能模块
...
src/engine/
  ...
  skill.ts             # 技能注册表 + registerAction/registerBeforeHook/registerAfterHook
  ...
  skills/              # 技能定义（每个文件一个技能，按声明注册表懒加载）
    registry.ts        # 声明聚合:武将 path + 装备卡名 + 系统/衍生表 → skillId → moduleSpec
    loaders.ts         # 按声明动态加载（vite-glob / Node 绝对路径 import 双环境）
    vite-glob.ts       # Vite/vitest 环境的静态路径映射（import.meta.glob）
    系统规则.ts         # 引擎级规则(濒死求桃/技能生命周期/装备兜底),跨模式共享
    开局.ts            # 开局流程(抽身份/选将/发牌),选将规则由 ruleset.opening 驱动
  ...
  rules/               # 规则包(游戏模式):模式级全局规则,core 经 registry 动态加载(ADR 0029)
    types.ts           # RulesetModule 契约(mode/onInit/checkGameOver/opening)
    registry.ts        # GameMode → dynamic import 表(零副作用)
    身份局.ts           # 系统规则组合 + 身份局胜负判定 + 标准选将(主公先行 7 候选)
    1v1.ts             # 两人对决:无主公特权,全员并行等额选将
...
| `registerSkillModule` / `skillLoaders` 手写表 | 已删除——技能模块由 `skills/registry.ts` 声明聚合 + `skills/loaders.ts` 动态加载 |
...
| [0030](./decisions/0030-skill-character-decouple.md) | 技能/角色/装备解耦 | **架构**：engine/{characters,skills,equipment} 分层 |
