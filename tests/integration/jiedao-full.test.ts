...
describe('借刀杀人完整流程', () => {
  ...
  it('P1出杀→P2被询问闪', async () => {
    ...
    if (h.state.pendingSlots.size > 0) { await P1.respond('杀', { cardId: 's2' }); await tick(); }
  ...
  it('P1不出杀→P0获得P1武器', async () => {
    ...
    if (h.state.pendingSlots.size > 0) await P0.pass(); // 无懈
    // forceKill 询问 → P1 不出杀
    if (h.state.pendingSlots.size > 0) { await P1.pass(); await tick(); }
    // P0 获得武器(进手牌,不是装备区)
  ...
  it('P1出杀→P2出闪→不扣血', async () => {
    ...
    if (h.state.pendingSlots.size > 0) { await P1.respond('杀', { cardId: 's2' }); await tick(); }
    // P2 被询问闪 → 出闪
    if (h.state.pendingSlots.size > 0) { await P2.respond('闪', { cardId: 'd1' }); await tick(); }
  ...
  it('P1出杀→P2不出闪→扣血', async () => {
    ...
    if (h.state.pendingSlots.size > 0) { await P1.respond('杀', { cardId: 's2' }); await tick(); }
    // P2 不出闪
    if (h.state.pendingSlots.size > 0) { await P2.pass(); await tick(); }
