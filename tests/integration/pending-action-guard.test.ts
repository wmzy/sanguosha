...
import { dispatch as engineDispatch, registerSkillsFromState, resetForTest } from '../../src/engine/create-engine';
...
describe('端到端:弃牌阶段重复 end 不死锁', () => {
  it('第一次 end 进入弃牌 pending; 第二次 end 被 validate 拒绝,不产生新 pending', async () => {
    ...
    engineDispatch(state, { skillId: '回合管理', actionType: 'end', ownerId: 0, params: {}, baseSeq: state.seq });
    await tick();
    ...
    // 仍只有 1 个弃牌 pending
    expect(state.pendingSlots.size).toBe(1);
    expect((state.pendingSlots.get(0)!.atom as { requestType?: string }).requestType).toBe('__弃牌');
  });
  ...
  it('弃牌 pending 期间 respond 正常工作(不被误拒)', async () => {
    ...
    engineDispatch(state, { skillId: '系统规则', actionType: 'respond', ownerId: 0, params: { cardIds: ['c0', 'c1'] }, baseSeq: state.seq });
    await tick();
