...
//   5. 禁止自选:选自己作目标被 validate 拒绝
//
// 与标版放权区别:
//   - 触发时机:标版「回合结束时」弃牌+选目标;界版「弃牌阶段开始时」弃牌+选目标
//   - 目标范围:均仅限其他角色(本实现不采用官方界限突破的"可为自己"设定)
//
...
describe('界放权', () => {
  ...
  it('禁止自选:选自己作目标被 validate 拒绝(不采用官方“可为自己”)', async () => {
    // 座次 0=界刘禅, 1=P1
    const c1 = mkCard('c1', '杀', '♠', '5');
    await harness.setup(
      createGameState({
    ...
    // 3. 选目标:选自己(0)→ 被 validate 拒绝(target===ownerId)
    LC.expectPending('请求回应');
    await LC.expectRejected({
      skillId: '界放权',
      actionType: 'respond',
      params: { target: 0 },
    });
