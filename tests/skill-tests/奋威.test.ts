...
describe('奋威', () => {
  ...
  it('南蛮入侵指定多目标 → 奋威令P3无效 → P3不受伤害,P1受伤害', async () => {
    ...
    // 南蛮目标 = allOthers from P2 = [P3(2), P1(0)](frame.params.targets 顺序)
    ...
    expect(cpPrompt.candidates).toEqual([2, 0]);
  ...
  it('铁索连环多目标 → 奋威令P3无效 → P3不横置,P4横置', async () => {
    ...
    // 无懈可击窗口(broadcast,逐目标)→ pass(无人打无懈) → 目标按当前状态自动切换
    await P1.pass(); // 无懈 target 2(P3)
    // P3 未横置 → 应自动横置,但被奋威令其无效(设横置被 cancel,维持未横置)
  ...
  it('铁索连环多目标 → 奋威令全部目标无效 → 无人横置', async () => {
    ...
    // 无懈窗口(逐目标)→ pass → 目标自动切换(P3/P4 均未横置,应横置,但都被奋威无效)
