...
//
// 验证:
//   1. 正面:无手牌时被杀 → 成为目标被 cancel → 无询问闪、不受伤害,杀牌正常进弃牌堆
...
describe('空城', () => {
  ...
  it('正面:无手牌被杀 → 成为目标被 cancel,不受伤害', async () => {
    ...
    // 空城机制:成为目标 atom 被 before-hook cancel(引擎记录 atomCancelled notify)
    const cancelledTypes = (
      harness.state.atomHistory as Array<{
        kind: string;
        eventType?: string;
