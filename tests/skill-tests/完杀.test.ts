...
describe('完杀', () => {
  ...
  it('贾诩回合内:第三方 P3 求桃被跳过,无人救则濒死者死亡', async () => {
    ...
    await harness.setup(
      createGameState({
        players: [
          ...
          // P2:1 血,即将濒死(带一张闪使询问闪走 normal、求桃走 silent,可逐步 pass)
          mkPlayer({ index: 1, name: 'P2', hand: ['p2s'], skills: ['闪'], health: 1, maxHealth: 4 }),
          // P3:有桃,但完杀下不能救
          mkPlayer({ index: 2, name: 'P3', hand: [peach.id], skills: ['桃'], health: 4 }),
        ],
        cardMap: { s1: slash, p1: peach, jxs: mkCard('jxs', '闪', '♥', '2'), p2s: mkCard('p2s', '闪', '♣', '3') },
    ...
    // 求桃顺序(模块 C:逆时针从当前回合贾诩起):贾诩 → P3(完杀跳过)→ P2(濒死者)。
    // 贾诩先被问(当前回合角色)→ 无桃 pass
    await JX.pass();
    await harness.waitForStable();

    // 完杀:P3 的求桃问询被 cancel(不创建 pending),流程直接跳到 P2(濒死者)
    // → 当前 pending 应是 P2(idx 1),而非 P3(idx 2)或贾诩(idx 0)
    const pendingSlot = [...harness.state.pendingSlots.values()][0];
    expect(pendingSlot).toBeTruthy();
    expect((pendingSlot.atom as { target: number }).target).toBe(1); // 问 P2(濒死者)

    // P2 也无桃,pass → 无人救援,P2 死亡
    await P2.pass();
    await harness.waitForStable();
  ...
  it('贾诩回合内:贾诩本人可对濒死者使用桃(完杀允许"你")', async () => {
    ...
    await harness.setup(
      createGameState({
        ...
        cardMap: { s2: slash, p2: peach, p2s: mkCard('p2s', '闪', '♣', '3') },
        currentPlayerIndex: 0,
        phase: '出牌',
        turn: { round: 1, phase: '出牌', vars: {} },
      }),
  ...
  it('贾诩回合内:濒死者本人可对自己使用桃', async () => {
    ...
    await harness.setup(
      createGameState({
        ...
        cardMap: { s3: slash, p3: peach, jxs: mkCard('jxs', '闪', '♥', '2') },
        currentPlayerIndex: 0,
        phase: '出牌',
        turn: { round: 1, phase: '出牌', vars: {} },
      }),
  ...
  it('非贾诩回合:第三方可正常救援(完杀仅在贾诩回合生效)', async () => {
    ...
    await harness.setup(
      createGameState({
        ...
        cardMap: { s4: slash, p4: peach, p2s: mkCard('p2s', '闪', '♣', '3'), p3s: mkCard('p3s', '闪', '♦', '6') },
    ...
    // 求桃顺序(模块 C:逆时针从当前回合 P3 起):P3 → P2(濒死)→ 贾诩
    // P3 先被问(当前回合角色)→ 无桃 pass
    await P3.pass();
    await harness.waitForStable();
    // 接下来问 P2(濒死者,idx 1——完杀未跳过任何角色)
