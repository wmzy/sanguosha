...
//
// 交互流程(confirm → 选目标 → 选牌 → [装备牌选代价]):
//   1. use action(confirm):出牌阶段发动(限一次)。
//   2. respond(界结姻/target,choosePlayer):选男性目标。
//   3. respond(界结姻/card,useCard):选一张手牌。
//   4. 所选为装备牌 → respond(界结姻/cost,chooseOption:弃手牌/置装备)。
//
// 验证:
...
//   4. 置装备代价·可替换原装备
//   5. 装备牌选择"弃手牌"代价 → 按弃置结算(非置装备)
//   6.【裁定】体力相等 → 双方均不摸牌不回血(仅消耗代价)
//   7. 限一次:同回合第二次 use 被拒绝
//   8. 非自己回合 → use 拒绝
//   9. 手牌为空 → use 拒绝
//   10. 无男性角色可选 → use 拒绝
//   11. 超时放弃选目标:技能失效但已消耗限一次
//   12. 超时放弃选牌:技能失效
//   13. target respond 选女性 → 拒绝(非法目标)
//   14. card respond 选不在手牌的牌 → 拒绝
//   15. choosePlayer/useCard pending 携带可序列化 candidates(跨进程契约)
...
describe('界结姻(OL 界限突破版)', () => {
  ...
  it('弃手牌:owner(2)<目标(4) → owner 回1(2→3),目标摸1,弃 h1', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', {}); // confirm 发动
    await P0.respond('界结姻', { target: 1 }); // 选目标 P1(曹操·男)
    await P0.respond('界结姻', { cardId: 'h1' }); // 选 h1(非装备→直接弃手牌)
  ...
  it('弃手牌:owner(3)>目标(1) → owner 摸1,目标回1(1→2)', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', {});
    await P0.respond('界结姻', { target: 1 });
    await P0.respond('界结姻', { cardId: 'h1' });
  ...
  it('置装备:owner(3)>目标(1) → 装备置入目标防具 + owner 摸1 + 目标回1', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', {});
    await P0.respond('界结姻', { target: 1 });
    await P0.respond('界结姻', { cardId: 'rw' }); // 装备牌
    await P0.respond('界结姻', { option: '置装备' }); // 选代价
  ...
  it('置装备:目标已有防具 → 替换(旧装备入弃牌堆)', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', {});
    await P0.respond('界结姻', { target: 1 });
    await P0.respond('界结姻', { cardId: 'rw2' });
    await P0.respond('界结姻', { option: '置装备' });
  ...
  // ─── 5. 装备牌选"弃手牌"代价 → 按弃置结算 ─────────────────
  it('装备牌选弃手牌代价 → 装备牌入弃牌堆,不置入目标', async () => {
    const state = buildState({
      p0Health: 3,
      p0MaxHealth: 3,
      p0Hand: ['rw'],
    ...
    await P0.triggerAction('界结姻', 'use', {});
    await P0.respond('界结姻', { target: 1 });
    await P0.respond('界结姻', { cardId: 'rw' });
    await P0.respond('界结姻', { option: '弃手牌' });

    expect(harness.state.players[1].equipment['防具']).toBeUndefined(); // 未置入
    expect(harness.state.zones.discardPile).toContain('rw'); // 直接弃置
    expect(harness.state.players[1].health).toBe(2); // 目标回1
  });
  ...
  it('限一次:已用过 → 第二次 use 被拒绝', async () => {
    ...
    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: {} });
  });

  // ─── 8. 非自己回合 → use 拒绝 ────────────────────────────
  it('非自己回合 → use 拒绝', async () => {
  ...
  it('放弃选目标:技能失效,限一次已消耗', async () => {
    ...
    expect(harness.state.players[0].hand).toEqual(['h1', 'h2', 'h3']);
    // 但限一次已消耗:第二次 use 被拒绝
    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: {} });
  });

  // ─── 12. 超时放弃选牌:技能失效 ──────────────────────────
  it('放弃选牌:技能失效,已选目标无效化', async () => {
    const state = buildState({ p0Health: 2, p0MaxHealth: 3, p1Health: 4, p1MaxHealth: 4 });
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');
  ...
  // ─── 13. target respond 选女性 → 拒绝 ────────────────────
  it('target respond 选女性(甄姬)→ 拒绝', async () => {
    const state = buildState({});
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');

    await P0.triggerAction('界结姻', 'use', {});
    await P0.expectRejected({ skillId: '界结姻', actionType: 'respond', params: { target: 2 } });
    // 仍停在选目标询问,可正常选 P1 继续
    await P0.respond('界结姻', { target: 1 });
    await P0.respond('界结姻', { cardId: 'h1' });
    expect(harness.state.players[0].health).toBe(3);
  ...
  // ─── 14. card respond 选不在手牌的牌 → 拒绝 ──────────────
  it('card respond 选不在手牌的牌 → 拒绝', async () => {
    const state = buildState({});
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');
    ...
    await P0.respond('界结姻', { target: 1 });
    // h4 不存在/不在手牌
    await P0.expectRejected({ skillId: '界结姻', actionType: 'respond', params: { cardId: 'h4' } });
    // 仍停在选牌询问,选 h1 继续
    await P0.respond('界结姻', { cardId: 'h1' });
    expect(harness.state.players[0].health).toBe(3);
  });

  // ─── 15. pending 携带可序列化 candidates(跨进程契约) ────
  it('choosePlayer/useCard pending 携带可序列化 candidates', async () => {
    const state = buildState({ p0Health: 2, p0MaxHealth: 3, p1Health: 4, p1MaxHealth: 4 });
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');

    await P0.triggerAction('界结姻', 'use', {});
    // 选目标 pending:choosePlayer 带 candidates(应为 [1],仅 P1 曹操男性)。
    // candidates 由投影层(buildView/toViewEvents)注入,在 view.pending.prompt 上读取。
    const targetPrompt = P0.view.pending?.prompt as { candidates?: number[] } | undefined;
    expect(targetPrompt?.candidates).toEqual([1]);

    await P0.respond('界结姻', { target: 1 });
    // 选牌 pending:useCard 的 candidates 在 prompt.cardFilter.candidates
    const cardPrompt = P0.view.pending?.prompt as
      | { cardFilter?: { candidates?: string[] } }
      | undefined;
    const handIds = harness.state.players[0].hand;
    expect(cardPrompt?.cardFilter?.candidates).toEqual(handIds);
