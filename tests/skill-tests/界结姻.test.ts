...
//
// 交互流程(distribute/allocate,无弹窗):
//   点「界结姻」→ 选一张牌(手牌或自己装备区装备)+ 点一名男性角色 → 确定。
//   use action 直接提交 allocation=[{target, cardIds:[id]}]。
//   代价由所选牌类型自动判定:装备牌 → 置入目标装备区;非装备牌 → 弃置该手牌。
//
// 验证:
//   1. 弃手牌(非装备)+ owner 体力较低 → owner 回1,目标摸1
//      (目标为满血男性,顺带覆盖【OL 差异】:界版不要求"已受伤")
//   2. 弃手牌 + owner 体力较高 → owner 摸1,目标回1
//   3. 置装备(手牌装备)+ owner 体力较高 → 装备置入目标 + owner 摸1 + 目标回1
//   4. 置装备·可替换原装备
//   5. 置装备·从 owner 装备区置入目标(核心需求:选装备区装备给目标)
//   6.【裁定】体力相等 → 双方均不摸牌不回血(仅消耗代价)
//   7. 限一次:同回合第二次 use 被拒绝
//   8. 非自己回合 → use 拒绝
//   9. 手牌与装备区均空 → use 拒绝
//   10. 无男性角色可选 → use 拒绝
//   11. 目标为女性 → use 拒绝(非法目标)
//   12. 牌不在手牌或装备区 → use 拒绝
//   13. allocation 格式(distribute/allocate UI 提交)同样生效
...
describe('界结姻(OL 界限突破版)', () => {
  ...
  it('弃手牌:owner(2)<目标(4) → owner 回1(2→3),目标摸1,弃 h1', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', { cardIds: ['h1'], target: 1 }); // h1 闪·非装备 → 弃手牌
  ...
  it('弃手牌:owner(3)>目标(1) → owner 摸1,目标回1(1→2)', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', { cardIds: ['h1'], target: 1 });
  ...
  // ─── 3. 置装备(手牌装备)+ owner 体力较高 → 装备置入目标 + 双向效果 ─
  it('置装备(手牌):owner(3)>目标(1) → 装备置入目标防具 + owner 摸1 + 目标回1', async () => {
    ...
    await P0.triggerAction('界结姻', 'use', { cardIds: ['rw'], target: 1 }); // rw 装备牌 → 置装备
  ...
  // ─── 4. 置装备·可替换原装备 ──────────────────────────
  it('置装备(手牌):目标已有防具 → 替换(旧装备入弃牌堆)', async () => {
    const state = buildState({
      p0Health: 3,
      p0MaxHealth: 3,
      p0Hand: ['rw2'],
    ...
    await P0.triggerAction('界结姻', 'use', { cardIds: ['rw2'], target: 1 });
  ...
  // ─── 5. 置装备·从 owner 装备区置入目标(核心需求)────────────
  // 选自己装备区的装备 → 卸下 → 置入目标装备区
  it('置装备(装备区):owner 装备区防具 → 置入目标空防具槽', async () => {
    const state = buildState({
      p0Health: 3,
      p0MaxHealth: 3,
      p0Hand: ['h1', 'h2'],
    ...
    await P0.triggerAction('界结姻', 'use', { cardIds: ['rw'], target: 1 }); // rw 来自装备区

    // rw 从 owner 装备区移到目标装备区
    expect(harness.state.players[1].equipment['防具']).toBe('rw');
    expect(harness.state.players[0].equipment['防具']).toBeUndefined();
    expect(harness.state.players[1].health).toBe(2); // owner(3)>目标(1) → 目标回1
    // owner 未弃手牌;rw 离开后 owner 摸1 → 手牌 = h1,h2 + 1 张摸牌
    expect(harness.state.players[0].hand).toContain('h1');
    expect(harness.state.players[0].hand).toContain('h2');
    expect(harness.state.players[0].hand.length).toBe(3);
  });
  ...
  it('限一次:已用过 → 第二次 use 被拒绝', async () => {
    ...
    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: { cardIds: ['h2'], target: 1 } });
  });
  ...
  it('非自己回合 → use 拒绝', async () => {
    ...
    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: { cardIds: ['h1'], target: 1 } });
    expect(harness.state.players[0].hand.length).toBe(3);
  });

  // ─── 9. 手牌与装备区均空 → use 拒绝 ──────────────────────
  it('手牌与装备区均空 → use 拒绝', async () => {
  ...
  it('无男性角色可选 → use 拒绝', async () => {
    // P1/P2 均为女性角色
    const state = buildState({ p1Character: '甄姬', p2Character: '大乔' });
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');
  ...
  // ─── 11. 目标为女性 → use 拒绝(非法目标)──────────────────
  it('目标为女性(甄姬)→ use 拒绝', async () => {
    const state = buildState({});
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');

    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: { cardIds: ['h1'], target: 2 } });
    // 状态不变
    expect(harness.state.players[0].hand.length).toBe(3);
    expect(harness.state.players[0].vars['界结姻/usedThisTurn']).toBeUndefined();
  });

  // ─── 12. 牌不在手牌或装备区 → use 拒绝 ────────────────────
  it('牌不在手牌或装备区 → use 拒绝', async () => {
    const state = buildState({});
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');

    // h4 不存在/不在手牌或装备区
    await P0.expectRejected({ skillId: '界结姻', actionType: 'use', params: { cardIds: ['h4'], target: 1 } });
    expect(harness.state.players[0].hand.length).toBe(3);
  });

  // ─── 13. allocation 格式(distribute/allocate UI 提交)同样生效 ─
  it('allocation 格式:allocation=[{target,cardIds}] 同样生效', async () => {
    const state = buildState({ p0Health: 2, p0MaxHealth: 3, p1Health: 4, p1MaxHealth: 4 });
    await harness.setup(state);
    const P0 = harness.player('界孙尚香');

    await P0.triggerAction('界结姻', 'use', {
      allocation: [{ target: 1, cardIds: ['h1'] }],
    });
