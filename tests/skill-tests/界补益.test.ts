...
//
// 验证(注:装备区只容纳 type==='装备牌',不存在“装备区锦囊牌”):
//   1. 端到端:P1 濒死 → P2(补益)选 P1 装备区装备牌 → 弃+回1
//   2. 端到端:选 P1 手牌(非基本牌)→ 弃+回1
//   3. 端到端:选 P1 手牌(基本牌)→ 无效果(不弃不回)
//   4. 端到端:不发动 → P1 继续求桃
//   5. 端到端:P1 无牌 → 不询问补益
//   6. respond validate:无 pending → 拒绝
//   7. respond validate:PICK 下 zone 缺失 → 拒绝
...
describe('界补益', () => {
  ...
  it('端到端:P1 濒死 → P2 选 P1 装备区装备牌 → 弃+回1', async () => {
    // 装备区只容纳 type==='装备牌';青釭剑为装备牌(非基本牌)→ 弃置+回血
