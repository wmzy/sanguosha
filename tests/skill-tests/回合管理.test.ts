...
describe('回合管理', () => {
  ...
  it('弃牌阶段:手牌超上限 → 弃牌 pending 出现,弃完后推进下家', async () => {
    // 准备:HP=2,手牌 5 张 > 手牌上限(=体力 2)→ 超限 3 张
    const state: GameState = createGameState({
      players: [
        makePlayer({
          index: 0,
    ...
    // P0 结束回合 → 进入弃牌阶段 → 手牌超限创建 __弃牌 pending
    await P1.triggerAction('回合管理', 'end', {});

    // 必须有 pending(__弃牌,target=0):弃牌阶段已实装手牌超限检查
    const pendingSlots = [...harness.state.pendingSlots.values()];
    expect(pendingSlots.length).toBeGreaterThan(0);
