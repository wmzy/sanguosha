...
describe('借刀杀人', () => {
  ...
  //    方天画戟的「最后一张手牌为杀可指定最多 3 目标」由 slashTargetMax provider 落实:
  //    P2 须在 skills 中声明方天画戟技能(测试手动装备不走装备 atom,需手动实例化),
  //    其 onInit 注册 provider;canUseSlash/借刀 respond.validate 据此放宽目标上限。
  ...
  it('P2(方天画戟, 仅 1 张杀) 被借刀 → 可对 B+C+D 多目标出杀,三目标均扣 1 血', async () => {
    ...
    const state = buildState({
      ...
      p2Skills: ['杀', '方天画戟'], // 实例化方天画戟技能(注册 slashTargetMax provider)
      p2Equipment: { 武器: 'wp1' },
      p3Hand: [],
      p3Skills: ['闪'],
