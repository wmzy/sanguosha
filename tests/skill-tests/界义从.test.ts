...
//   2. 单元:体力 > 2 时(=4、=3),vars['距离/防御修正'] 未设(含上界边界 3)
//   3. 单元:体力 ≤ 2 时(=2、=1),vars['距离/防御修正'] = 1(=2 验证 ≤ 而非 <;=1 验证 ≤ 而非 ==)
//   4. 单元:体力=2 时,他人到 P0 的 effectiveDistance = 座位距离 +1(防御修正可观测)
//   5. 触发(实际 dispatch):P0(义从,徒手)对距离 2 的 P3 出杀 → 命中(-1 距离生效)
//   6. 触发:受伤害掉到 2 血 → after hook 同步防御修正 = 1
//   7. 触发:回复到 3 血 → after hook 同步防御修正被清除
//   8. 触发:失去体力掉到 2 血 → after hook 同步防御修正 = 1
...
describe('界义从', () => {
  ...
  it('单元:体力=3(刚高于阈值)时,vars[距离/防御修正] 未设', async () => {
    // 上界边界:阈值 LOW_HEALTH_THRESHOLD=2,3 应不激活,区别于 ≤3 的错误实现
    await harness.setup(build4PlayerState({ p0Skills: ['界义从'], p0Health: 3 }));
    expect(harness.state.players[0].vars['距离/防御修正']).toBeUndefined();
  });
  ...
  // ─────────────────────────────────────────────────────────────
  // 4. 单元:体力 ≤ 2 时,他人到 P0 的距离 +1(防御修正可观测)
  // ─────────────────────────────────────────────────────────────
  it('单元:体力=2 时,他人到 P0 的 effectiveDistance = 座位距离 +1', async () => {
    // 4 人局 P2(座次 2)→ P0(座次 0)座位距离 2;P0 低血防御 +1 → 实际 3
    await harness.setup(build4PlayerState({ p0Skills: ['界义从'], p0Health: 2 }));
    expect(harness.state.players[0].vars['距离/防御修正']).toBe(1);
    expect(effectiveDistance(harness.state, 2, 0)).toBe(3);
  });

  // ─────────────────────────────────────────────────────────────
  // 5. 触发:P0(徒手,范围 1)对距离 2 的 P3 出杀 → 命中(进攻-1 生效)
  // ─────────────────────────────────────────────────────────────
  it('触发:P0(义从)徒手对距离 2 的 P3 出杀 → 命中(进攻-1 缩距到 1)', async () => {
