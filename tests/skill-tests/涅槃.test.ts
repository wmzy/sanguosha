// 涅槃(庞统·限定技)行为测试:
//   1. 濒死时发动涅槃:弃所有牌+判定区、解除连环、摸3张、回复至3体力,且不死亡
//   2. 不发动涅槃 → 进入求桃流程,无人救则死亡
//   3. 限定技:用过一次后,再次濒死不再触发
...
describe('涅槃', () => {
  ...
  it('濒死发动涅槃:弃所有牌、解除连环、摸3张、回复至3体力', async () => {
    ...
    await PT.pass();
    // 此时进入濒死流程 → 涅槃询问发动(请求回应 pending)
    await harness.waitForStable();
    // 庞统 confirm 发动涅槃
    await PT.respond('涅槃', { choice: true });
    await harness.waitForStable();
  ...
  it('不发动涅槃 → 求桃无人救 → 死亡', async () => {
    ...
    await harness.waitForStable();
    // 涅槃询问 → 不发动
    await PT.respond('涅槃', { choice: false });
    await harness.waitForStable();
  ...
  it('限定技:用过一次后再次濒死不再触发涅槃', async () => {
    ...
    // 涅槃不应再触发(used 已设):无 涅槃/confirm 的请求回应 pending
    // 应直接进入求桃流程(桃/求桃),而非涅槃询问
    const requestTypes = [...harness.state.pendingSlots.values()].map(
      (s) => (s.atom as Record<string, unknown>).requestType,
    );
    expect(requestTypes).not.toContain('涅槃/confirm');
