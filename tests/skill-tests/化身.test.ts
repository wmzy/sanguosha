// 化身(左慈/界左慈·群)行为测试:
//   标版(左慈):skillId='化身',2 张牌,二选一(更换/不更换)
...
//   界版(界左慈):同一 skillId='化身'(官方技能名),3 张牌,三选一(替换/移去/不操作)
//     1-3. 同标版(初始 3 张)
//     4. 选 1(替换)→ 卸载旧技能 + 亮出另一张 + 获得新技能
//     5. 选 2(移去)count=1/2 → 池大小不变,展示牌与技能保留
//     6. 边界:池大小为 2 时,count=2 被拒
//
...
describe('化身(界左慈)', () => {
  ...
  it('首次回合开始:抽 3 张未登场武将牌,亮出一张,获得一个技能', async () => {
    await harness.setup(
      createGameState({
        players: [
          mkPlayer({ index: 0, name: '界左慈', character: '界左慈', skills: ['化身', '新生'] }),
          mkPlayer({ index: 1, name: '曹操', character: '曹操', skills: [] }),
        ],
        cardMap: {},
        currentPlayerIndex: 0,
  ...
  it('第二次自己回合开始:询问选择行动(1/2/3)', async () => {
    ...
    ZUO.expectPending('请求回应');
    const slot = harness.state.pendingSlots.get(0);
    expect((slot!.atom as Record<string, unknown>).requestType).toBe('化身/选择行动');

    // 选择不操作(3)
    await ZUO.respond('化身', { option: '3' });
    await harness.waitForStable();
    harness.processAllEvents();
  ...
  it('选 2 count=1:移去 1 张并获得 1 张新化身牌,展示牌和技能保留', async () => {
    ...
    // 选 2(移去)
    await ZUO.respond('化身', { option: '2' });
    await harness.waitForStable();
    harness.processAllEvents();
    // 紧接着询问移除数量
    ZUO.expectPending('请求回应');
    const slot = harness.state.pendingSlots.get(0);
    expect((slot!.atom as Record<string, unknown>).requestType).toBe('化身/移除数量');
