...
describe.skip('damage.type', () => {
  ...
  it('默认 type 为 normal，写入 server event payload', () => {
    ...
    expect(events[0].atom.type).toBe('造成伤害');
    expect(events[0].atom).toMatchObject({ amount: 1 });
  });
  ...
  it.each(['fire', 'thunder'] as const)('damageType=%s 写入 payload.type', (t) => {
    ...
    const { logEntries: events } = applyAtoms(s0, [
      { type: '造成伤害', target: 'P1', amount: 1, source: 'P2', damageType: t },
    ]);
    expect(events[0].atom).toMatchObject({ damageType: t });
  });

  it('toEvents payload 保留 damageType=fire 字段', () => {
    const s0 = setHealth(createTestGame(), 'P1', 4);
    const { logEntries: events } = applyAtoms(s0, [
      { type: '造成伤害', target: 'P1', amount: 2, source: 'P2', damageType: 'fire' },
    ]);
    expect(events[0].atom).toHaveProperty('damageType', 'fire');
  });
});
