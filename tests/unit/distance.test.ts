...
describe('distance', () => {
  ...
  it('weapon extends attack range', () => {
    ...
    const player = {
      ...game.players[0],
      equipment: {
        weapon: { id: '青龙偃月刀-♠-5', name: '青龙偃月刀', type: '装备牌' as const, subtype: '武器' as const, suit: '♠' as const, rank: '5' as const, description: '', range: 3 },
      },
    };
  ...
  it('+1 horse increases distance to player', () => {
    const game = createTestGame();
    game.players[1].equipment.horsePlus = { id: '的卢-♠-K', name: '的卢', type: '装备牌' as const, subtype: '防御马' as const, suit: '♠' as const, rank: 'K' as const, description: '' };
