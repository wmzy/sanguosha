...
describe('eventToServerOp', () => {
  ...
  it('damage event 描述含伤害来源、目标、数值', () => {
    const op = eventToServerOp(ev('damage', { target: P2, amount: 1, source: P1, cardId: 'c1' }), state);
    expect(op).not.toBeNull();
    expect(op!.type).toBe('damage');
  ...
  it('damage event 无 source 时描述以目标开头', () => {
    const op = eventToServerOp(ev('damage', { target: P2, amount: 2 }), state);
    expect(op!.description).toMatch(/^P2/);
  });

  it('heal event 描述含目标、回复量、当前体力', () => {
    state.players[P2].health = 2;
    const op = eventToServerOp(ev('heal', { target: P2, amount: 1 }), state);
  ...
  it('cardsDiscarded event 描述含卡名列表', () => {
    const op = eventToServerOp(ev('cardsDiscarded', { player: P1, cardIds: ['c1', 'c2'] }), state);
    expect(op!.type).toBe('discard');
    expect(op!.description).toContain('P1');
    expect(op!.description).toContain('杀、闪');
  });

  it('cardsDiscarded 空牌列表描述仅含数量', () => {
    const op = eventToServerOp(ev('cardsDiscarded', { player: P1, cardIds: [] }), state);
    expect(op!.description).toBe('P1弃了0张牌');
  });

  it('equip event 描述含卡名 + 槽位中文', () => {
    const op = eventToServerOp(ev('equip', { player: P1, cardId: 'c1', slot: 'weapon' }), state);
    expect(op!.type).toBe('equip');
    expect(op!.description).toContain('武器');
    expect(op!.description).toContain('杀');
  });

  it('unequip event 描述含槽位中文', () => {
    const op = eventToServerOp(ev('unequip', { player: P1, slot: 'armor' }), state);
    expect(op!.description).toBe('P1卸下了防具');
  });

  it('setPhase event 描述含阶段与玩家', () => {
    const op = eventToServerOp(ev('setPhase', { phase: '出牌', player: P1 }), state);
    expect(op!.type).toBe('phaseChange');
    expect(op!.description).toContain('出牌');
    expect(op!.description).toContain('P1');
  });

  it('nextPlayer event 描述含目标、轮次、回合数', () => {
    const op = eventToServerOp(ev('nextPlayer', { from: P1, to: P2, turnNumber: 2, round: 1 }), state);
    expect(op!.type).toBe('turnChange');
    expect(op!.description).toContain('P2');
  ...
  it('turnStart event 返回 null（与 nextPlayer 重复）', () => {
    const op = eventToServerOp(ev('turnStart', { player: P1 }), state);
    expect(op).toBeNull();
  });

  it('kill event 描述含玩家 + 来源', () => {
    const op = eventToServerOp(ev('kill', { player: P2, source: P1 }), state);
    expect(op!.description).toContain('P2阵亡');
    expect(op!.description).toContain('P1');
  });

  it('dying event 描述含濒死玩家', () => {
    const op = eventToServerOp(ev('dying', { player: P2 }), state);
    expect(op!.description).toBe('P2濒死');
  });

  it('addSkill event 描述含技能名', () => {
    const op = eventToServerOp(ev('addSkill', { player: P1, skillId: '奸雄' }), state);
    expect(op!.type).toBe('skillActivate');
    expect(op!.description).toContain('奸雄');
  });

  it('未知事件类型返回 null', () => {
    const op = eventToServerOp(ev('moveCard', { cardId: 'c1' }), state);
    expect(op).toBeNull();
  });
});
...
describe('eventToPlayerOp — 视角裁剪', () => {
  ...
  it('draw event：drawer 看到具体卡名', () => {
    const op = eventToPlayerOp(
      ev('draw', { player: P1, count: 2, cards: ['c1', 'c2'] }),
      state,
      P1,
    );
    expect(op).not.toBeNull();
  ...
  it('damage event 对所有人一致', () => {
    const e = ev('damage', { target: P2, amount: 1, source: P1 });
...
describe('actionToOp', () => {
  ...
  it('startGame → gameStart', () => {
    const op = actionToOp({ type: 'startGame' }, state);
    expect(op!.type).toBe('gameStart');
    expect(op!.description).toBe('游戏开始');
  });

  it('playCard 描述含卡名与目标', () => {
    const op = actionToOp({ type: 'playCard', player: P1, cardId: 'c1', target: P2 }, state);
    expect(op!.type).toBe('play');
    expect(op!.description).toContain('P1');
    expect(op!.description).toContain('杀');
    expect(op!.description).toContain('P2');
  });
  ...
  it('playCard 无 target 时描述不含目标', () => {
    const op = actionToOp({ type: 'playCard', player: P1, cardId: 'c1' }, state);
    expect(op!.description).not.toContain('目标');
  });

  it('respond 描述含打出卡名', () => {
    const op = actionToOp({ type: 'respond', player: P1, cardId: 'c2' }, state);
    expect(op!.type).toBe('play');
    expect(op!.description).toContain('闪');
  });

  it('useSkill 描述含技能名', () => {
    const op = actionToOp({ type: 'useSkill', player: P1, skillId: '奸雄' }, state);
    expect(op!.description).toContain('奸雄');
  });

  it('endTurn / discard / toggleAutoSkipWuxie 返回 null（被 server events 覆盖）', () => {
    expect(actionToOp({ type: 'endTurn', player: P1 }, state)).toBeNull();
    expect(actionToOp({ type: 'discard', player: P1, cardIds: [] }, state)).toBeNull();
    expect(actionToOp({ type: 'toggleAutoSkipWuxie' }, state)).toBeNull();
  });
});
...
describe('GameLogger', () => {
  ...
  it('recordBatch 单条 event → serverOps / 每个 playerOps 各加 1 条，seq 递增', () => {
    ...
    const result = logger.recordBatch(
      null,
      [ev('damage', { target: P2, amount: 1, source: P1 }, 'e1')],
      state,
    );
  ...
  it('多次 recordBatch 之间 seq 连续递增', () => {
    ...
    logger.recordBatch(null, [ev('damage', { target: P2, amount: 1, source: P1 }, 'e1')], state);
    logger.recordBatch(null, [ev('heal', { target: P2, amount: 1 }, 'e2')], state);
    logger.recordBatch(null, [ev('setPhase', { phase: '出牌', player: P1 }, 'e3')], state);
  ...
  it('draw 事件按 player 拆分：drawer 看 cards，他人只看 count', () => {
    ...
    logger.recordBatch(null, [ev('draw', { player: P1, count: 2, cards: ['c1', 'c2'] }, 'e1')], state);
  ...
  it('action 触发：startGame 走 serverOps 与所有 playerOps', () => {
    ...
    logger.recordBatch({ type: 'startGame' }, [], state);
    expect(logger.getServerOps()).toHaveLength(1);
    expect(logger.getServerOps()[0].type).toBe('gameStart');
    expect(logger.getPlayerOps(P1)[0].type).toBe('gameStart');
    expect(logger.getPlayerOps(P2)[0].type).toBe('gameStart');
  });
  ...
  it('rebuildFromLog 从 serverLog 重建（断线重连）', () => {
    ...
    const serverLog = [
      ev('damage', { target: P2, amount: 1, source: P1 }, 'e1', 100),
      ev('heal', { target: P2, amount: 1 }, 'e2', 200),
    ];
