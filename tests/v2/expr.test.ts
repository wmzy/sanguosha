...
import { resolve, checkCondition } from '@engine/v2/expr';
import type { Expr, Condition, SkillContext, GameState } from '@engine/v2/types';
...
function stateWithVars(vars: Record<string, unknown>): GameState {
  ...
  return {
    ...
    players: {
      ...s.players,
      P1: { ...s.players.P1, vars: vars as Record<string, import('@engine/v2/types').Json> },
    },
  };
}
...
describe('checkCondition', () => {
  ...
  it('returns false for unknown condition shape', () => {
    expect(checkCondition({ unknown: 'test' } as unknown as import('@engine/v2/types').Condition, state)).toBe(false);
  });
