...
import { applyAtom, atomToEvents } from '@engine/v2/atom';
...
describe('judge', () => {
  it('apply: 从牌堆顶抽判定牌并置入弃牌堆', () => {
    ...
    const result = applyAtom(state, { type: 'judge', player: 'P1' });
  ...
  it('apply: 牌堆为空时洗弃牌堆再判定', () => {
    ...
    const result = applyAtom(emptied, { type: 'judge', player: 'P1' });
  ...
  it('toEvents: 生成 judge 事件', () => {
    const state = createTestGame();
    const [serverEvent] = atomToEvents(state, { type: 'judge', player: 'P1' });
    expect(serverEvent.type).toBe('judge');
...
describe('kill', () => {
  it('apply: 将玩家标记为阵亡', () => {
    const state = createTestGame();
    const result = applyAtom(state, { type: 'kill', player: 'P1', source: 'P2' });
    expect(result.players.P1.info.alive).toBe(false);
  });

  it('toEvents: 生成 kill 事件', () => {
    const state = createTestGame();
    const [serverEvent] = atomToEvents(state, { type: 'kill', player: 'P1', source: 'P2' });
    expect(serverEvent.type).toBe('kill');
    expect((serverEvent.payload as Record<string, unknown>).player).toBe('P1');
