...
describe('useAnimationState · 摸牌检测(newCardIds)', () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  // 注:hook 首次挂载时 prevHandRef 初值为 [],会把初始手牌全部视为"新摸"。
  // 故以空手挂载,再通过 rerender 摸牌,得到干净的新增信号。

  it('视角手牌新增 id 时记录到 newCardIds', () => {
    const { result, rerender } = renderHook(({ view }) => useAnimationState(view, 0), {
      initialProps: { view: makeView({ p0Hand: ['a', 'b'] }) },
    });
    // 先清空首挂产生的初始 newCardIds
    act(() => {
      vi.advanceTimersByTime(550);
    });

    // 摸了一张 c
    rerender({ view: makeView({ p0Hand: ['a', 'b', 'c'] }) });
    expect([...result.current.newCardIds]).toEqual(['c']);
  });

  it('摸多张牌时全部记录', () => {
    const { result, rerender } = renderHook(({ view }) => useAnimationState(view, 0), {
      initialProps: { view: makeView({ p0Hand: ['a'] }) },
    });
    act(() => {
      vi.advanceTimersByTime(550);
    });
    rerender({ view: makeView({ p0Hand: ['a', 'b', 'c'] }) });
    expect([...result.current.newCardIds].sort()).toEqual(['b', 'c']);
  });

  it('摸牌动画窗口(550ms)结束后自动清除 newCardIds', () => {
    const { result, rerender } = renderHook(({ view }) => useAnimationState(view, 0), {
 initialProps: { view: makeView({ p0Hand: [] }) },
    });
    // 空手挂载:newCardIds 为空
    expect(result.current.newCardIds.size).toBe(0);
    rerender({ view: makeView({ p0Hand: ['a', 'b'] }) });
    expect(result.current.newCardIds.size).toBe(2);
    act(() => {
      vi.advanceTimersByTime(550);
    });
    expect(result.current.newCardIds.size).toBe(0);
  ...
  it('perspectiveIdx 越界(无该玩家)时不报错', () => {
    const { result, rerender } = renderHook(({ view }) => useAnimationState(view, 99), {
      initialProps: { view: makeView({ p0Hand: ['a'] }) },
    });
    expect(() => rerender({ view: makeView({ p0Hand: ['a', 'b'] }) })).not.toThrow();
    expect(result.current.newCardIds.size).toBe(0);
  });
});
