...
describe('青釭剑 v3（无视防具）', () => {
  ...
  it('source 有青釭剑时，钩子注入 penetrateArmor 上下文变量', () => {
    ...
    const s0 = withWeapon(setHealth(createTestGame(), 'P1', 4), 'P2', 'qinggang');
    ...
    expect(events.some((e) => e.type === 'setCtxVar')).toBe(true);
  });
  ...
  it('source 未装备青釭剑，钩子不触发', () => {
    ...
    expect(events.some((e) => e.type === 'setCtxVar')).toBe(false);
  });
});
