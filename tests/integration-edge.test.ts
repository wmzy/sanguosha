...
describe.skip('边界: 牌堆抽空 + reshuffle', () => {
  it('空牌堆抽牌后，卡牌从弃牌堆洗回牌堆', () => {
    ...
    const r2 = engine(state, { type: '结束回合', player: 'P1' });
    // 应该能正常切换玩家（走不需要弃牌的分支）
...
describe('边界: 多人局距离', () => {
  ...
  it('装备-1马后 P1→P3 距离变为 1', () => {
    ...
    const r1 = engine(state, { type: '打出一张牌', player: 'P1', cardId: equipId });
  ...
  it('顺手牵羊检查距离=1，对距离为2的目标应当报错', () => {
    ...
    const result = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: trickId, target: 'P3',
    });
  ...
  it('顺手牵羊对距离=1的目标可以正常使用', () => {
    ...
    const r1 = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: trickId, target: 'P2',
    });
    expect(r1.error).toBeUndefined();
    // 弹出 trickResponse 窗口（目标可出无懈可击）
    expect(r1.state.pending?.type).toBe('响应窗口');
    expect((r1.state.pending! as { window: { type: string } }).window.type).toBe('trickResponse');
  });
});
...
describe('边界: 濒死/死亡流程', () => {
  it('杀将目标体力降至0时触发濒死 pending', () => {
    ...
    // 应该触发濒死窗口
    const isDying = r2.state.pending?.type === '濒死窗口' ||
      r2.events?.some(e => e.type === '濒死');
    if (!isDying) {
      // ⚠️ BUG 可能性: 濒死未触发
...
describe('边界: 卡牌目标验证', () => {
  it('杀不能对自己使用', () => {
    ...
    const result = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: killId, target: 'P1',
    });
  ...
  it('杀不能对已死亡的玩家使用', () => {
    ...
    const result = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: killId, target: 'P2',
    });
    expect(result.error).toBeTruthy();
  });
  ...
  it('不能使用不存在的手牌', () => {
    ...
    const result = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: 'nonexistent-card',
    });
    expect(result.error).toBeTruthy();
  });
});
...
describe('边界: AOE/决斗', () => {
  it('南蛮入侵触发 aoeResponse 窗口链', () => {
    ...
    const r1pre = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: trickId,
    });
    ...
    expect(r1.pending?.type).toBe('响应窗口');
    if (r1.pending?.type === '响应窗口') {
    ...
    const r2pre = engine(r1, { type: '打出', player: 'P2', cardId: killInP2 });
    ...
    expect(r2.pending?.type).toBe('响应窗口');
    if (r2.pending?.type === '响应窗口') {
      expect(r2.pending.window.type).toBe('aoeResponse');
      expect(r2.pending.window.defender).toBe('P3');
    }

    // P3 不出杀 → 受 1 点伤害
    const r3pre = engine(r2, { type: '打出', player: 'P3' });
    ...
    expect(r3.pending?.type).toBe('响应窗口');
    if (r3.pending?.type === '响应窗口') {
    ...
    const r4 = engine(r3, { type: '打出', player: 'P4' });
  ...
  it('AOE 顺序：从使用者逆时针开始（座位非 0 的使用者）', () => {
    ...
    const r0 = engine(state, { type: '打出一张牌', player: 'P3', cardId: trickId });
    ...
    const r2pre = engine(r1, { type: '打出', player: 'P4', cardId: dodgeP4 });
    const r2 = passAllTrickResponders(r2pre.state);
    if (r2.pending?.type === '响应窗口') {
      expect(r2.pending.window.type).toBe('aoeResponse');
      expect(r2.pending.window.defender).toBe('P1');
    }
    const r3pre = engine(r2, { type: '打出', player: 'P1' });
    const r3 = passAllTrickResponders(r3pre.state);
    if (r3.pending?.type === '响应窗口') {
      expect(r3.pending.window.type).toBe('aoeResponse');
  ...
  it('顺手牵羊全流程：trickResponse → selectCard → 获得目标手牌', () => {
    ...
    // 1. 使用顺手牵羊
    const r1 = engine(state, {
      type: '打出一张牌', player: 'P1', cardId: stealId, target: 'P2',
    });
    expect(r1.error).toBeUndefined();
    // 源牌进弃牌堆
    ...
    expect(r2.state.pending?.type).toBe('选择牌');
    if (r2.state.pending?.type === '选择牌') {
      expect(r2.state.pending.mode).toBe('获得');
    ...
    // 3. 选择一张牌
    const targetCardId = (r2.state.pending as { cardIds: string[] }).cardIds[0];
    const r3 = engine(r2.state, { type: '打出', player: 'P1', cardIds: [targetCardId] });
    expect(r3.error).toBeUndefined();
    // 该牌从 P2 转移到 P1
  ...
  it('顺手牵羊被无懈可击取消', () => {
    ...
    // P2 出无懈可击 → 取消顺手牵羊
    const r2 = engine(r1.state, { type: '打出', player: 'P2', cardId: wuxieId });
    expect(r2.error).toBeUndefined();
    // 无懈可击进弃牌堆
    ...
    expect(r3.pending?.type).toBe('出牌阶段');
    // P2 的牌没有被拿走
  ...
  it('装备武器后攻击范围更新', () => {
    ...
    if (beforeKill) {
      const rBefore = engine(state, {
        type: '打出一张牌', player: 'P1', cardId: beforeKill, target: 'P3',
      });
    ...
    const rEquip = engine(state, { type: '打出一张牌', player: 'P1', cardId: equipId });
    ...
    if (afterKill) {
      const rAfter = engine(rEquip.state, {
        type: '打出一张牌', player: 'P1', cardId: afterKill, target: 'P3',
      });
...
describe('边界: 死亡玩家行为限制', () => {
  it('已死亡玩家出牌应报错', () => {
    ...
    if (!killId) return;
    const result = engine(state, {
      type: '打出一张牌', player: 'P2', cardId: killId, target: 'P1',
    });
...
describe('边界: 状态不变量', () => {
  it('cardMap 中的每张牌都在某处（手牌/牌堆/弃牌堆/装备区/判定区）', () => {
    ...
    for (const p of state.playerOrder) {
      ...
      // 装备区
      if (player.equipment) {
        Object.values(player.equipment).forEach((id: string | null) => {
...
describe('卡牌边界', () => {
  it('顺手牵羊 steal mode 经过 trickResponse → resolveSelectCard 正确放入偷牌者手牌', () => {
    ...
    const step1 = engine(state, { type: '打出一张牌', player: 'P1', cardId, target: 'P2' });
    expect(step1.error).toBeUndefined();
    expect(step1.state.pending!.type).toBe('响应窗口');

    // Step2: 过 trickResponse（无懈不出 → 放行）
    const step2 = engine(step1.state, { type: '打出', player: 'P2' });
    expect(step2.error).toBeUndefined();
    expect(step2.state.pending!.type).toBe('选择牌');

    // Step3: 选择偷哪张
    const selected = p2HandBefore[0];
    const step3 = engine(step2.state, { type: '打出', player: 'P1', cardIds: [selected] });
  ...
  it('顺手牵羊距离检查装备-1马后距离0可用', () => {
    ...
    expect(result.error).toBeUndefined();
    expect(result.state.pending?.type).toBe('响应窗口');
  });
  ...
  it('装备新武器时旧武器进入弃牌堆', () => {
    ...
    const r1 = engine(state, { type: '打出一张牌', player: 'P1', cardId: weapon1 });
    expect(r1.error).toBeUndefined();
    state = r1.state;
    state = injectEquipCard(state, 'P1', '麒麟弓', '武器', 5);
    const weapon2 = findCardInHand(state, 'P1', '麒麟弓')!;
    const r2 = engine(state, { type: '打出一张牌', player: 'P1', cardId: weapon2 });
    expect(r2.error).toBeUndefined();
    expect(r2.state.zones.discardPile.includes(weapon1)).toBe(true);
    expect(r2.state.players['P1'].equipment.武器).toBe(weapon2);
  });
  ...
  it('牌堆+弃牌堆全空时 draw 不报错', () => {
    ...
    const handBefore = state.players['P1'].hand.length;
    const result = engine(state, { type: '打出一张牌', player: 'P1', cardId });
    expect(result.error).toBeUndefined();
  ...
  it('过河拆桥出牌后 pending 期间 sourceCard 已进入弃牌堆', () => {
    ...
    const r1 = engine(state, { type: '打出一张牌', player: 'P1', cardId, target: 'P2' });
  ...
  it('顺手牵羊装备武器后 validate 允许但 handler 拒绝距离>1', () => {
    ...
    const stealId = findCardInHand(state, 'P1', '顺手牵羊')!;
    const rFail = engine(state, { type: '打出一张牌', player: 'P1', cardId: stealId, target: 'P3' });
