...
- **青釭剑**：① 杀被【闪】抵消后临时卸载的目标防具技能未恢复（原仅挂「造成伤害后」，漏「被抵消」退出路径，导致防具永久泄漏）→ 补「被抵消」after hook 恢复；② 对顺手牵羊/过河拆桥等非杀目标牌也误卸载防具且不恢复 → 加「杀」来源守卫。
- **雌雄双股剑**：原为简化占位实现（注释自承「不对性别判断、效果同时执行」）。① 补异性性别校验（`getGender`）；② 效果改为由**目标二选一**（弃一张手牌，或令使用者摸一张牌），全座次注册 respond（符合刚烈/反间既定模式）。
- **寒冰剑**：触发条件原仅查手牌，遗漏装备区（规则为「手牌或装备区有牌」）→ 纳入装备区；弃牌改为由使用者逐张选择。
- **麒麟弓**：在决斗/南蛮入侵/万箭齐发/火攻等非杀来源的伤害上误触发 → 加「杀」来源守卫（仅「你使用【杀】造成伤害时」触发）。
- **藤甲**：被顺手牵羊/反馈「获得」后陈旧 hook 残留（获得 atom 不触发移除技能，防具已易主仍生效）→ 触发前动态校验防具槽仍是藤甲（陷阱#8）。
- **白银狮子**：失去装备回血原仅挂「卸下」，漏掉弃置/获得/换装替换三条路径 → 覆盖四条互斥路径并加门禁防止重复回血。

其余 8 个技能（诸葛连弩/青龙偃月刀/贯石斧/丈八蛇矛/方天画戟/仁王盾/八卦阵/马匹技能）审查无 bug。全部补/改测试，`npm run test:all` 3314 passed（唯一失败的 `tests/client/skillDisplay.test.ts` 为已知客户端 flaky，与本次无关）。

### Fixed — 卡牌图片花色/点数与文件名大面积错配

`public/packs/base/card/`（运行时手牌大图）中，除少数「首张」外的绝大多数卡牌图片，其左上角点数/花色角标与文件名（=`src/shared/deck.ts` 真相源）不一致——同名不同花色点数的物理牌共用同一张图（字节完全相同）。根因：packs/base 曾由 `sync-cards-local.ts`（按牌名复制通用图到所有变体）+ `migrate-to-packs.ts`（搬到 packs/base）填充，每个牌名只保留首张角标，其余变体被折叠。最严重的几组甚至跨牌名共用一图：`大宛-K-♠ = 紫骍-K-♦ = 赤兔-5-♥`、`爪黄飞电-K-♥ = 的卢-5-♣ = 绝影-5-♠ = 骅骝-K-♦`。
...
调试房间创建后不再立即开局,进入「配置+准备」阶段。房主(调试房间任意玩家)可配置房间名、操作倒计时秒数、将池预设、初始手牌数。所有玩家(调试房间逐座次)准备后可开始。配置模型对普通/调试房间通用,前端先只接调试房间。

- **`RoomConfig`** (`src/server/protocol.ts`): 新增 `{ name, timeoutSec, charPool, handSize }` + `normalizeRoomConfig` 规范化 + `DEFAULT_ROOM_CONFIG`。
...
- **引擎 `GameState.config`** (`src/engine/types.ts`+`create-engine.ts`): `GameConfig.timeoutSec` 注入 `state.config`;新增 `resolveTimeoutMs(state, baseSeconds)` helper,`createAndAwaitSlot` 与各 atom view 层统一应用 timeoutSec(0=无限)。
